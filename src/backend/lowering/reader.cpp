...
IRVal Reader::ReadValue(const IRVal &val, Emitter &emitter) {
  switch (val.kind) {
  case 'C':
    return ReadConst(val, emitter);
  case 'V':
    return ReadLoad(val, emitter);
  case 'T':
    return val;
  default:
    throw std::runtime_error(std::string("Unknown IRVal kind: ") + val.kind);
  }
}

IRVal Reader::ReadInstr(const IRInstr &instr, Emitter &emitter) {
  if (instr.op == "BINARY")
    return ReadBinary(instr, emitter);
  if (instr.op == "UNARY")
    return ReadUnary(instr, emitter);
  if (instr.op == "CALL")
    return ReadCall(instr, emitter);
  if (instr.op == "INTERP")
    return ReadInterp(instr, emitter);
  throw std::runtime_error("Unhandled opcode in Reader: " + instr.op);
}

std::vector<IRVal> Reader::ReadValues(const std::vector<IRVal> &vals,
                                      Emitter &emitter) {
...
IRVal Reader::ReadConst(const IRVal &val, Emitter &emitter) {
  if (val.kind != 'C') {
    throw std::runtime_error("ReadConst called with non-const IRVal (kind: " +
                             std::string(1, val.kind) + ")");
  }
