...
class SemanticAnalyzer:
    ...
    def _AnalyzeFunctionDef(self, Node: FunctionDefStmt) -> None:
        ...
        if Existing:
            self.Errors.append(
                RedefinitionError(
                    f"Function '{NameStr}' already defined",
                    Node.Name.Line,
                    Node.Name.Column,
                )
            )
        ...
        elif len(Node.ReturnTypes) > 0 and not (
            len(Node.ReturnTypes) == 1 and Node.ReturnTypes[0].Lexeme in {"none", "nil"}
        ):
            self.Errors.append(
                MissingReturnError(
    ...
    def _AnalyzeReturn(self, Node: ReturnStmt) -> None:
        if not self.CurrentFunction:
            self.Errors.append(
                InvalidReturnError("return statement outside of function")
            )
            return
    ...
    def _InferExprType(self, Expr) -> str:
        ...
        if isinstance(Expr, IdentifierExpr):
            NameStr = (
                Expr.Name.Lexeme if hasattr(Expr.Name, "Lexeme") else str(Expr.Name)
            )
        ...
        if isinstance(Expr, CallExpr):
            CalleeName = (
                Expr.Callee.Lexeme
                if hasattr(Expr.Callee, "Lexeme")
                else str(Expr.Callee)
            )
        ...
        if isinstance(Node, IdentifierExpr):
            if Node.Name:
                ...
                if not Sym:
                    self.Errors.append(
                        NameResolutionError(
                            f"Undefined identifier '{Node.Name.Lexeme}'",
                            Node.Name.Line,
                            Node.Name.Column,
                        )
                    )
            ...
            if Node.Callee:
                ...
                if not Sym or Sym.SymbolType != "function":
                    self.Errors.append(
                        NameResolutionError(
                            f"Undefined function '{Node.Callee.Lexeme}'",
                            Node.Callee.Line,
                            Node.Callee.Column,
                        )
                    )
