...
def CheckReturns(Nodes: List[ASTNode]):
    ...
    for Node in Nodes:
        if isinstance(Node, FunctionDefStmt):
            ExpectReturn = len(Node.ReturnTypes) > 0 and not (
                len(Node.ReturnTypes) == 1
                and Node.ReturnTypes[0].Lexeme in {"none", "nil"}
            )
            if ExpectReturn and not Node.ReturnExpr:
                ...
                Errors.append(
                    MissingReturnError(
                        "missing return expression for function", Line, Column
                    )
                )
    return Errors
