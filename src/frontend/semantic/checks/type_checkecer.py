...
def CheckTypes(Nodes: List[ASTNode]):
	Errors = []
	for Node in Nodes:
		if isinstance(Node, FunctionDefStmt) and Node.ReturnTypes and isinstance(Node.ReturnExpr, LiteralExpr):
			...
			if Lexeme is not None:
				...
				if Actual not in Allowed:
					...
					Errors.append(TypeMismatchError(f"function return type '{Actual}' not in declared return union", Line, Column))
	return Errors
