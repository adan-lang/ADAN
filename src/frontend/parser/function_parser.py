...
def ParseFunctionDef(State: Parser):
    ...
    if not NameToken.Lexeme.isidentifier():
        raise ValueError(
            f"Expected identifier after 'function' at line {NameToken.Line}, column {NameToken.Column}: got '{NameToken.Lexeme}'"
        )
    ...
    while not State.IsAtEnd() and State.Peek().Lexeme != ")":
        ...
        if not ParamName.Lexeme.isidentifier():
            raise ValueError(
                f"Expected parameter name at line {ParamName.Line}, column {ParamName.Column}: got '{ParamName.Lexeme}'"
            )
        ...
        if not State.IsAtEnd() and State.Peek().Lexeme == ",":
            ...
            Token = State.Peek()
            raise ValueError(
                f"Expected ',' or ')' in parameter list at line {Token.Line}, column {Token.Column}: got '{Token.Lexeme}'"
            )
